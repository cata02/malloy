...
import { buildQueryMaterializationSpec, shouldMaterialize, } from './materialization/utils';
...
export function makeQueryModel(modelDef: ModelDef | undefined, eventStream?: EventStream): QueryModel {
...
export class QueryModelImpl implements QueryModel, ModelRootInterface {
    ...
    constructor(modelDef: ModelDef | undefined, readonly eventStream?: EventStream) {
    ...
    getStructFromRef(structRef: StructRef, sourceArguments: Record<string, Argument> | undefined, prepareResultOptions?: PrepareResultOptions): QueryStruct {
        ...
        if (typeof structRef === 'string') {
            ...
            if (sourceArguments !== undefined) {
                return new QueryStruct(ret.structDef, sourceArguments, ret.parent ? { struct: ret.parent } : { model: this }, prepareResultOptions);
            }
            return ret;
        }
        const refWithArgs = sourceArguments
            ? ({
                ...(structRef as any),
                arguments: {
            ...
            : (structRef as any);
        return new QueryStruct(refWithArgs, sourceArguments, { model: this }, prepareResultOptions);
    }
    loadQuery(query: Query, stageWriter: StageWriter | undefined, prepareResultOptions?: PrepareResultOptions, emitFinalStage = false, isJoinedSubquery = false): QueryResults {
        const malloy = '';
        ...
        const q = QueryQuery.makeQuery(turtleDef, queryStruct, stageWriter, isJoinedSubquery, (name: string) => this.structs.get(name));
        ...
        return {
            ...
            sourceArguments: ret.sourceArguments ?? query.sourceArguments,
        };
    }
    ...
    compileQuery(query: Query, prepareResultOptions?: PrepareResultOptions, finalize = true): CompiledQuery {
        process.stdout.write(`\n>>> [compileQuery] CALLED! hasSourceArguments=${!!query.sourceArguments}, keys=${query.sourceArguments
            ? Object.keys(query.sourceArguments).join(',')
            : 'none'} <<<\n`);
        let newModel: QueryModel | undefined;
        const addDefaultRowLimit = this.addDefaultRowLimit(query, prepareResultOptions?.defaultRowLimit);
        ...
        const sourceArguments = query.sourceArguments ??
        ...
        return {
            ...
            materialization: shouldMaterialize(query.annotation)
                ? buildQueryMaterializationSpec(query.location?.url, query.name, prepareResultOptions?.materializedTablePrefix)
                : undefined,
            structs: ret.structs,
            sourceExplore,
    ...
    async searchIndex(connection: Connection, explore: string, searchValue: string, limit = 1000, searchField: string | undefined = undefined): Promise<SearchIndexResult[] | undefined> {
...
export function getResultStructDefForQuery(model: ModelDef, query: Query): SourceDef {
...
export function getResultStructDefForView(source: SourceDef, view: TurtleDef): SourceDef {
