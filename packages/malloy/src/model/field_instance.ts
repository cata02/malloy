...
import { isIndexSegment, isRawSegment, isJoined, expressionIsAnalytic, hasExpression, isAtomic, mergeUniqueKeyRequirement, } from './malloy_types';
...
export class FieldInstanceField implements FieldInstance {
    ...
    constructor(public f: QueryField, public fieldUsage: InstanceFieldUsage, public parent: FieldInstanceResult, public readonly drillExpression: Malloy.Expression | undefined) { }
    ...
    static registerExpressionCompiler(compiler: (resultSet: FieldInstanceResult, context: QueryStruct, expr: Expr, state?: GenerateState) => string) {
    ...
    getSQL() {
        ...
        if (isScalarField(this.f)) {
            exp = caseGroup(this.parent.groupSet > 0
...
export class FieldInstanceResult implements FieldInstance {
    ...
    constructor(public turtleDef: TurtleDef, public parent: FieldInstanceResult | undefined) {
    ...
    addField(as: string, field: QueryField, usage: InstanceFieldUsage, drillExpression: Malloy.Expression | undefined) {
    ...
    fields(fn: undefined | ((field: FieldInstanceField) => boolean) = undefined): FieldInstanceField[] {
    ...
    addStructToJoin(qs: QueryStruct, uniqueKeyRequirement: UniqueKeyRequirement): void {
        ...
        if (join) {
            join.uniqueKeyRequirement = mergeUniqueKeyRequirement(join.uniqueKeyRequirement, uniqueKeyRequirement);
            return;
        }
    ...
    getUngroupPartitions(ungroupSet: UngroupSet | undefined): FieldInstanceField[] {
        ...
        for (const fieldName of ungroupSet?.fields || []) {
            if (inScopeFieldNames.indexOf(fieldName) === -1) {
                throw new Error(`${ungroupSet?.type}(): unknown field name "${fieldName}" or name not in scope.`);
            }
        }
        return ret;
    }
